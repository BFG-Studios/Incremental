...
var sieveImg;//button for the sieve function
var xImg;
var baseImg;
var shopImg;
var sellImg;
...
var seedChance = stats[0]; // variable for the chance to get a seed when harvesting a plant
var failChance = 20 - stats[1]; // variable for the chance to fail when harvesting a plant
var goldBonus = stats[2]; //the bonus when selling a plant
var gold = 0;
var selected = 3;
var seedT;
var plantT;
...
function startFunc(){
	...
	sieveImg = new Image; //pic for sieve
	sieveImg.src = "../img/Sieve.png";
...
function onClick(e){
	...
	switch (cG){
		case 0:
			...
			if (xClick > 560 && xClick < 620 && yClick > 30 && yClick < 60){
					var rand = Math.floor(Math.random() * 100) + 1; // variable generated every time Sieve is clicked
					if(rand < 5){
						window.alert("You found a seed!");// lets the player know they found a seed
						seeds[Math.floor(Math.random() * 3)] += 1;//randomly assigns 1 seed to the player
					}//checks if the player found a seed
							...
							for (j = 0; j < 4; j++){
								if (yClick > farmPlot[i][j].y && yClick < farmPlot[i][j].y+64){
									...
									if (farmPlot[i][j].harvest == true){ //checks if the square is harvestable, takes the plant into the inventory and returns to it's start state
										...
										if (Math.floor(Math.random() * 100) + 1 > failChance){
											plants[farmPlot[i][j].seed] += 1;
											farmPlot[i][j].harvest = false;
											farmPlot[i][j].seed = 3;
											farmPlot[i][j].img = baseImg;
											farmPlot[i][j].grow = 0;
											if (Math.floor(Math.random() * 100) + 1 < seedChance){
												seeds[farmPlot[i][j].seed] += 1;
											}/* has a chance to gain an extra seed of the type harvested*/
										}//checks if the harvest failed
										else{
											window.alert("That harvest failed, increase your INT to decrease the likeliness of this occuring again.");
											farmPlot[i][j].harvest = false;
											farmPlot[i][j].seed = 3;
											farmPlot[i][j].img = baseImg;
											farmPlot[i][j].grow = 0;
			...
			if(xClick > 490 && xClick < 550){ // check if they clicked the buy button
				if (yClick > 60 && yClick < 90){
					if (selected != 3 && gold > 24){ // if they have a plant selected, and enough gold to buy a plant, buy it
						seeds[selected]++;
						gold -= 25;
					}
				}
			...
			if(xClick > 490 && xClick < 550){ // check if they clicked the eat button
				if (yClick > 90 && yClick < 120){
					if (selected != 3 && plants[selected] > 0){ // if they have a plant selected, and a plant to eat, eat it
						plants[selected] -= 1;
						stats[selected] += 1;
							if (stats[selected] == stats[0]){
								seedChance = stats[selected];
							}
							if (stats[selected] == stats[1]){
								failChance = 20 - stats[selected];
							}
							if (stats[selected] == stats[2]){
								goldBonus = stats[selected];
							}
					}
				}
			}
...
function playerMove(){
	if (sdcPlayer.left == true && sdcPlayer.x > 0){
		sdcPlayer.x -= sdcPlayer.hspeed + (stats[0]/4)/* adds 1 for every 4 dex to horizontal move speed*/;
...
{
	if(leftPressed)
		pltPlayer.V_X = -(3 + (stats[0]/4));//Increases the player move speed by 1 every 4 points in dex
	if(rightPressed)
		pltPlayer.V_X = 3 + (stats[0]/4);//Increases the player move speed by 1 every 4 points in dex
...
function render(){
	...
	switch (cG){
		case 0:
			renderer.drawImage(sieveImg,560,30);
			renderer.drawImage(shopImg,490,1);
