...
var goldBonus = stats[2] * 10;
var seedChance = stats[1];
var extraChance = stats[0];
/*
	goldBonus is used to alter the sell and purchase of Seeds
	seedChance is used to alter the chance to recover a seed when harvesting
	extraChance is used to alter the chance for a second crop when harvesting
*/
					...
					for (j = 0; j < 4; j++){
						if (yClick > farmPlot[i][j].y && yClick < farmPlot[i][j].y+64){
							...
							if (farmPlot[i][j].harvest == true){ //checks if the square is harvestable, takes the plant into the inventory and returns to it's start state
								...
								}/* has a chance to gain a seed of the type harvested*/
	...
	if(xClick > 490 && xClick < 550){ // check if they clicked the buy button
		if (yClick > 60 && yClick < 90){
			if (selected != 3 && gold > 0){ // if they have a plant selected, and enough gold to buy a plant, buy it
				seeds[selected]++;
				gold -= 25 - stats[2];/* /*incorporated chr to decrease purchase value by 1 per chr*/
			}
		}
	}
